...
var router = function(app) {
	app.post('/images', function(req, res, next){
		...
		db.insert(input).then(function(data){
			if(req.files != null) {
				var tmp_path = req.files.attachement.path;
				var target_path = './public/images/' + data.id;
				fs.rename(tmp_path, target_path, function(err) {
					if(err) {
						return res.send(res_json);
					} else {
						return db.get({'_id': data.id});//query by id
					}
				})
			}
		}).then(function(data){
	...
	app.del('/images/:id', function(req, res, next){
		console.log('req.parmas.id: ' + req.params.id);
		var json = {'_id': req.params.id};
		db.get(json).then(function(data){
			var res_json = JSON.parse(JSON.stringify(constant.RES));
